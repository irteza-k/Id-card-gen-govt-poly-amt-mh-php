...
include("db_connect.php"); 
  ...
  if(isset($_POST['resetpass'])){
	  ...
	  $memail=mysqli_real_escape_string($db,$_POST['memail']);
	    ...
	    $minstitution =mysqli_real_escape_string($db,$_POST['minstitution']);
		   ...
		   $check="SELECT * FROM Users WHERE id='$id' ";
						  ...
						  $found=mysqli_num_rows($checks);
							  ...
							  {              $f=move_uploaded_file ($orgtmpName,'images/'.$orgName);
									  ...
									  $quer = "UPDATE Users SET Firstname='$mfname',Sirname='$msname',Mtitle='$mtitle',Email='$memail',Staffid='$mid',Rank='$rank',Department='$minstititution',Picname='$orgName' WHERE id='$id' ";
 ...
 if(isset($_POST["bulk"]))
	{
		...
		while(($filesop = fgetcsv($handle, 1000, ",")) !== false)
		{
			...
			$Picname=$filesop[7];
				 $count++;
			  if($count>1){ 
			$query = "INSERT INTO Users (Firstname,Sirname,Mtitle,Email,Staffid,Rank,Department,Picname) ".
                            "VALUES ('$mfname','$msname', '$mtitle','$mphone','$sid','$rank','$minstititution','$Picname')";
                                 $db->query($query) or die('Error1, query failed');	
